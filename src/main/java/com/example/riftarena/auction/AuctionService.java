...
public class AuctionService{
 ...
 @SuppressWarnings("unchecked") public Map<String,Object> create(Map<String,Object>p){List<Number> raw=(List<Number>)p.get("memberIds");if(raw==null||raw.size()!=10)throw new IllegalArgumentException("참가자 10명이 필요합니다.");List<Long>ids=new ArrayList<>();for(Number n:raw)ids.add(n.longValue());if(new HashSet<>(ids).size()!=10)throw new IllegalArgumentException("중복 참가자가 있습니다.");Long b=num(p.get("blueHostId")),r=num(p.get("redHostId"));if(b==null||r==null||b.equals(r)||!ids.contains(b)||!ids.contains(r))throw new IllegalArgumentException("서로 다른 호스트를 선택하세요.");List<Map<String,Object>> originalMembers=members.byIds(ids);if(originalMembers.size()!=10)throw new IllegalArgumentException("활성 멤버 10명을 찾을 수 없습니다.");List<Map<String,Object>> ms=PositionCrowdingAdjuster.adjust(originalMembers);int base=intv(p.get("baseToken"),1000),ratePct=(int)Math.round(doublev(p.get("adjustRate"),.5)*100);Room room=new Room(code(),b,r,base,ratePct);for(Map<String,Object>m:ms)room.members.put(((Number)m.get("memberId")).longValue(),new LinkedHashMap<>(m));int bs=score(room,b),rs=score(room,r),ded=(int)Math.round(Math.abs(bs-rs)*(ratePct/100.0));room.blueTokens=base;room.redTokens=base;if(bs>rs)room.blueTokens=Math.max(100,base-ded);else if(rs>bs)room.redTokens=Math.max(100,base-ded);room.blueTeam.add(new Pick(b,0,true));room.redTeam.add(new Pick(r,0,true));ids.stream().filter(x->!x.equals(b)&&!x.equals(r)).sorted(Comparator.comparingInt(x->score(room,x))).forEach(room.queue::add);log(room,"SYSTEM","실시간 경매방이 생성되었습니다. "+crowdingSummary(ms));rooms.put(room.code,room);return state(room.code);}
