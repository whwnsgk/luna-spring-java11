...
public class AuctionService{
 ...
 @SuppressWarnings("unchecked") public Map<String,Object> create(Map<String,Object>p){cleanupRooms();List<Number> raw=(List<Number>)p.get("memberIds");if(raw==null||raw.size()!=10)throw new IllegalArgumentException("참가자 10명이 필요합니다.");List<Long>ids=new ArrayList<>();for(Number n:raw)ids.add(n.longValue());if(new HashSet<>(ids).size()!=10)throw new IllegalArgumentException("중복 참가자가 있습니다.");Long b=num(p.get("blueHostId")),r=num(p.get("redHostId"));if(b==null||r==null||b.equals(r)||!ids.contains(b)||!ids.contains(r))throw new IllegalArgumentException("서로 다른 호스트를 선택하세요.");List<Map<String,Object>> originalMembers=members.byIds(ids);if(originalMembers.size()!=10)throw new IllegalArgumentException("활성 멤버 10명을 찾을 수 없습니다.");List<Map<String,Object>> ms=PositionCrowdingAdjuster.adjust(originalMembers);int base=intv(p.get("baseToken"),1000),ratePct=(int)Math.round(doublev(p.get("adjustRate"),.5)*100);Room room=new Room(code(),b,r,base,ratePct);for(Map<String,Object>m:ms)room.members.put(((Number)m.get("memberId")).longValue(),new LinkedHashMap<>(m));room.initialCandidateScoreTotal=ids.stream().filter(x->!x.equals(b)&&!x.equals(r)).mapToInt(x->score(room,x)).sum();room.initialCandidateCount=8;int bs=score(room,b),rs=score(room,r),ded=(int)Math.round(Math.abs(bs-rs)*(ratePct/100.0));room.blueTokens=base;room.redTokens=base;if(bs>rs)room.blueTokens=Math.max(100,base-ded);else if(rs>bs)room.redTokens=Math.max(100,base-ded);room.blueTeam.add(new Pick(b,0,true));room.redTeam.add(new Pick(r,0,true));ids.stream().filter(x->!x.equals(b)&&!x.equals(r)).sorted(Comparator.comparingInt(x->score(room,x))).forEach(room.queue::add);log(room,"SYSTEM","실시간 경매방이 생성되었습니다. "+crowdingSummary(ms));rooms.put(room.code,room);return state(room.code);}
 ...
 private void settle(Room r,String team,int price){
  ...
  r.eventMemberScore=score(r,id);
  r.eventAverageScore=r.initialCandidateCount==0?0:(int)Math.round(r.initialCandidateScoreTotal/(double)r.initialCandidateCount);
  r.eventFairPrice=fairPrice(r,id);
  broadcast(r);
  r.currentId=null;
 ...
 private void event(Room r,String type,String team,String title,String message,String memberName,int price){r.eventSeq++;r.eventType=type;r.eventTeam=team;r.eventTitle=title;r.eventMessage=message;r.eventMemberName=memberName;r.eventPrice=price;r.eventFairPrice=0;r.eventMemberScore=0;r.eventAverageScore=0;}
 private int fairPrice(Room r,Long memberId){
  if(memberId==null||r.initialCandidateScoreTotal<=0)return 10;
  double totalAuctionBudget=r.baseToken*2.0;
  return Math.max(10,(int)Math.round(totalAuctionBudget*score(r,memberId)/r.initialCandidateScoreTotal));
 }
 ...
 private String crowdingSummary(List<Map<String,Object>> members){
  List<String> adjusted=new ArrayList<>();
  for(Map<String,Object> member:members){
   int penalty=intv(member.get("positionCrowdingPenalty"),0);
   if(penalty>0)adjusted.add(String.valueOf(member.get("realName"))+" -"+penalty);
  }
  return adjusted.isEmpty()?"포지션 과밀 보정 없음":"포지션 과밀 보정 · "+String.join(", ",adjusted);
 }
 ...
 static class Room{String code,status="WAITING";Long blueHostId,redHostId,currentId;int blueTokens,redTokens,round,currentBid,eventPrice,eventFairPrice,eventMemberScore,eventAverageScore,baseToken,initialCandidateScoreTotal,initialCandidateCount;long eventSeq;String bidTeam,turnTeam,surrendered,eventType,eventTeam,eventTitle,eventMessage,eventMemberName;boolean openingBid;Instant deadline,startDeadline,createdAt=Instant.now(),completedAt;Map<Long,Map<String,Object>>members=new LinkedHashMap<>();List<Long>queue=new ArrayList<>();List<Pick>blueTeam=new ArrayList<>(),redTeam=new ArrayList<>();Map<String,WebSocketSession>sessions=new LinkedHashMap<>();Map<String,String>roleClientIds=new HashMap<>();Map<String,String>spectatorKeys=new HashMap<>();Set<String>passedTeams=new HashSet<>();List<Map<String,Object>>logs=new ArrayList<>();Room(String c,Long b,Long r,int base,int rate){code=c;blueHostId=b;redHostId=r;baseToken=base;}}
}
