...
import java.time.LocalDateTime;import java.time.format.DateTimeParseException;import java.util.*;import org.springframework.stereotype.Service;import org.springframework.transaction.annotation.Transactional;import com.example.riftarena.member.*;import com.example.riftarena.champion.*;
@Service
public class MatchService{
 private final MatchMapper mapper;private final MemberService members;private final ChampionService champions;public MatchService(MatchMapper mapper,MemberService members,ChampionService champions){this.mapper=mapper;this.members=members;this.champions=champions;}
 @Transactional public Map<String,Object> create(Map<String,Object>p){Prepared x=prepare(p);p.putAll(x.match);mapper.insertMatch(p);Long id=((Number)p.get("matchId")).longValue();insertPlayers(id,x.players,x.winner);members.recalculateBalanceScores(playerIds(x.players));return detail(id);}
 @Transactional public Map<String,Object> update(Long id,Map<String,Object>p){detail(id);Prepared x=prepare(p);x.match.put("matchId",id);x.match.put("updatedBy",str(p.get("updatedBy"),"사용자"));x.match.put("updateReason",str(p.get("updateReason"),"경기 기록 수정"));mapper.updateMatch(x.match);mapper.deletePlayers(id);insertPlayers(id,x.players,x.winner);mapper.insertChangeLog(x.match);members.recalculateBalanceScores(playerIds(x.players));return detail(id);}
